...
const input = {
    language: 'Solidity',
    sources: {
        'SimpleToken.sol': { content: source }
    },
    settings: {
        outputSelection: {
...
fs.writeFileSync(path.join(__dirname, 'SimpleToken_bytecode.txt'), bytecode);
fs.writeFileSync(path.join(__dirname, 'SimpleToken_abi.json'), abi);

console.log('SimpleToken compiled!');
